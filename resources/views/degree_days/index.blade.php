...
@section('title', 'Dán Nhãn Tổng Nhiệt Hữu Hiệu (Degree-Days)')
...
@section('content')
    <x-page-header title="Dán Nhãn Tổng Nhiệt Hữu Hiệu (Degree-Days)">
        <x-slot:breadcrumbs>
            <a href="{{ url('/dashboard') }}"><i class="bi bi-house-door"></i> Trang chủ</a>
            <span>/</span>
            <span>IoT & Quan trắc</span>
            <span>/</span>
            <span class="text-primary fw-bold">Dán nhãn Tổng Nhiệt hữu hiệu</span>
        </x-slot:breadcrumbs>

        <x-slot:actions>
            <span class="badge bg-primary-subtle text-primary border border-primary-subtle px-3 py-2 rounded-pill fw-semibold">
                <i class="bi bi-shield-check me-1"></i> Điều tra thực địa & Chuẩn hóa mô hình
            </span>
        </x-slot:actions>
    </x-page-header>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-xl-3 col-md-6">
            <div class="survey-card p-3 d-flex align-items-center gap-3">
                <div class="rounded-4 bg-success-subtle text-success p-3 d-flex align-items-center justify-content-center" style="width: 48px; height: 48px; font-size: 22px;">
        ...
        <div class="col-xl-3 col-md-6">
            <div class="survey-card p-3 d-flex align-items-center gap-3">
                <div class="rounded-4 bg-warning-subtle text-warning p-3 d-flex align-items-center justify-content-center" style="width: 48px; height: 48px; font-size: 22px;">
        ...
        <div class="col-xl-3 col-md-6">
            <div class="survey-card p-3 d-flex align-items-center gap-3">
                <div class="rounded-4 bg-danger-subtle text-danger p-3 d-flex align-items-center justify-content-center" style="width: 48px; height: 48px; font-size: 22px;">
        ...
        <div class="card-header bg-white border-bottom p-0">
            <ul class="nav nav-tabs survey-nav-tabs px-3" id="surveyTabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="tab-survey-form-btn" data-bs-toggle="tab" data-bs-target="#tab-survey-form" type="button" role="tab">
                        <i class="bi bi-plus-circle-fill text-primary me-1.5"></i> Thực Hiện Khảo Sát Mới
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="tab-survey-history-btn" data-bs-toggle="tab" data-bs-target="#tab-survey-history" type="button" role="tab">
        ...
        <div class="card-body p-4">
            <div class="tab-content" id="surveyTabsContent">
                ...
                <div class="tab-pane fade show active" id="tab-survey-form" role="tabpanel">
                    @if ($allowedStations->isEmpty())
                        <div class="alert alert-warning border-0 rounded-4 d-flex align-items-center gap-3 p-3">
                            ...
                            <div>
                                <h6 class="fw-bold mb-1">Chưa có trạm quan trắc nào được phân quyền cho tài khoản của bạn</h6>
                                <p class="mb-0 text-muted small">
                                    Bạn chưa sở hữu hoặc quản lý vườn/trạm quan trắc nào trong hệ thống. Vui lòng liên hệ Quản trị viên để gán quyền trạm quan trắc trước khi thực hiện dán nhãn thực địa.
                                </p>
                            </div>
                        </div>
                    @else
                        <form action="{{ route('degree-days.surveys.store') }}" method="POST" enctype="multipart/form-data" id="degreeDaysSurveyForm">
                            @csrf

                            <!-- SECTION 1: THÔNG TIN KHẢO SÁT -->
                            <div class="mb-4">
                                <div class="d-flex align-items-center gap-2 mb-3">
                                    <span class="badge bg-primary text-white rounded-circle p-1" style="width: 24px; height: 24px; display: inline-flex; align-items: center; justify-content: center; font-size: 12px;">1</span>
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="datetime-local" class="form-control form-control-sm rounded-3 @error('surveyed_at') is-invalid @enderror" 
                                            id="surveyed_at" name="surveyed_at" 
                                            value="{{ old('surveyed_at', now()->format('Y-m-d\TH:i')) }}" required>
                                        @error('surveyed_at')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        <small class="text-muted" style="font-size: 11px;">Mặc định lấy thời điểm hiện tại</small>
                                    </div>

                                    <!-- Trạm quan trắc (chỉ trạm user có quyền) -->
                                    <div class="col-md-5">
                                        <label for="monitoring_station_id" class="form-label fw-semibold text-dark small mb-1">
                                            Trạm / Khu vực / Vườn <span class="text-danger">*</span>
                                        </label>
                                        <select class="form-select form-select-sm rounded-3 @error('monitoring_station_id') is-invalid @enderror" 
                                            id="monitoring_station_id" name="monitoring_station_id" required>
                                            @foreach ($allowedStations as $st)
                                                <option value="{{ $st->id }}" {{ old('monitoring_station_id', $selectedStationId) == $st->id ? 'selected' : '' }}>
                                                    [{{ $st->code }}] {{ $st->name }} ({{ $st->garden->name ?? 'Vùng trồng' }})
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('monitoring_station_id')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        <small class="text-muted" style="font-size: 11px;">Chỉ hiển thị các trạm thuộc quyền sở hữu của bạn</small>
                                    </div>

                                    <!-- Người khảo sát (Tự động lấy user) -->
                                    <div class="col-md-3">
                                        <label class="form-label fw-semibold text-dark small mb-1">Người khảo sát</label>
                                        <input type="text" class="form-control form-control-sm rounded-3 bg-light text-muted fw-medium" 
                                            value="{{ Auth::user()->name }} ({{ Auth::user()->username }})" readonly>
                                        <small class="text-muted" style="font-size: 11px;">Tự động gắn mã User ID của bạn</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- SECTION 2: DỮ LIỆU IOT GẦN NHẤT (READ-ONLY) -->
                            <div class="mb-4">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <div class="d-flex align-items-center gap-2">
                                        <span class="badge bg-primary text-white rounded-circle p-1" style="width: 24px; height: 24px; display: inline-flex; align-items: center; justify-content: center; font-size: 12px;">2</span>
                                        <h6 class="fw-bold text-dark mb-0">Dữ Liệu Vi Khí Hậu IoT Gần Nhất (Snapshot Tự Động)</h6>
                                    </div>
                                    <span class="badge bg-light text-muted border font-monospace" id="iot-recorded-time-badge" style="font-size: 11px;">
                                        <i class="bi bi-clock me-1"></i> <span id="iot-time-text">{{ $initialSnapshot['recorded_at_display'] ?? 'Đang tải...' }}</span>
                                    </span>
                                </div>
                                ...
                                <div class="p-3 bg-light rounded-4 border">
                                    <div class="row g-2 text-center" id="iot-kpi-container">
                                        ...
                                        <div class="col-6 col-md-4 col-lg-2">
                                            <div class="iot-kpi-box">
                                                <div class="text-muted small" style="font-size: 11.5px;"><i class="bi bi-thermometer-half text-danger"></i> Nhiệt độ</div>
                                        ...
                                        <div class="col-6 col-md-4 col-lg-2">
                                            <div class="iot-kpi-box">
                                                <div class="text-muted small" style="font-size: 11.5px;"><i class="bi bi-droplet-half text-info"></i> Độ ẩm KK</div>
                                        ...
                                        <div class="col-6 col-md-4 col-lg-2">
                                            <div class="iot-kpi-box">
                                                <div class="text-muted small" style="font-size: 11.5px;"><i class="bi bi-cloud-rain-fill text-primary"></i> Lượng mưa</div>
                                        ...
                                        <div class="col-6 col-md-4 col-lg-2">
                                            <div class="iot-kpi-box">
                                                <div class="text-muted small" style="font-size: 11.5px;"><i class="bi bi-wind text-secondary"></i> Tốc độ gió</div>
                                        ...
                                        <div class="col-6 col-md-4 col-lg-2">
                                            <div class="iot-kpi-box">
                                                <div class="text-muted small" style="font-size: 11.5px;"><i class="bi bi-moisture text-success"></i> Độ ẩm đất</div>
                                        ...
                                        <div class="col-6 col-md-4 col-lg-2">
                                            <div class="iot-kpi-box">
                                                <div class="text-muted small" style="font-size: 11.5px;"><i class="bi bi-brightness-high text-warning"></i> Ánh sáng</div>
                                    ...
                                    <div class="text-muted text-center small mt-2" style="font-size: 11px;">
                                        <i class="bi bi-info-circle me-1"></i> Dữ liệu trên được hệ thống tự động tìm và gán cố định theo thời gian khảo sát thực địa của trạm.
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                <div class="d-flex align-items-center gap-2 mb-3">
                                    <span class="badge bg-primary text-white rounded-circle p-1" style="width: 24px; height: 24px; display: inline-flex; align-items: center; justify-content: center; font-size: 12px;">3</span>
                                ...
                                <div class="row g-3 mb-4">
                                    <div class="col-md-6">
                                        <div class="object-type-card selected d-flex align-items-center gap-3" id="card-type-pest" onclick="selectObjectType('pest')">
                                            <input class="form-check-input mt-0 fs-5" type="radio" name="object_type" id="type_pest" value="pest" checked>
                                            <div>
                                                <h6 class="fw-bold text-dark mb-0"><i class="bi bi-bug-fill text-warning me-1.5"></i> Sâu Đục Cuống Quả</h6>
                                                <small class="text-muted">Quan sát trứng, sâu non, nhộng hoặc bướm trưởng thành</small>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="object-type-card d-flex align-items-center gap-3" id="card-type-disease" onclick="selectObjectType('disease')">
                                            <input class="form-check-input mt-0 fs-5" type="radio" name="object_type" id="type_disease" value="disease">
                                            <div>
                                                <h6 class="fw-bold text-dark mb-0"><i class="bi bi-virus text-danger me-1.5"></i> Bệnh Hại Cây Trồng</h6>
                                                <small class="text-muted">Quan sát bệnh trên lá, hoa, quả hoặc cành thân</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div id="fields-pest-group" class="p-3 bg-light rounded-4 border mb-3">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-6">
                                            <label for="development_stage" class="form-label fw-semibold text-dark small mb-1">
                                                Giai đoạn phát triển của sâu <span class="text-danger">*</span>
                                            </label>
                                            <select class="form-select form-select-sm rounded-3" id="development_stage" name="development_stage">
                                                ...
                                                <option value="adult">Trưởng thành (Vũ hóa)</option>
                                        ...
                                        <div class="col-md-6">
                                            <label for="severity_pest" class="form-label fw-semibold text-dark small mb-1">
                                                Mức độ phát sinh <span class="text-danger">*</span>
                                            </label>
                                            <select class="form-select form-select-sm rounded-3" id="severity_pest" name="severity">
                                        ...
                                        <!-- Số lượng quan sát được (Chọn theo khoảng) -->
                                        <div class="col-12">
                                            <label class="form-label fw-semibold text-dark small mb-1.5">
                                                Số lượng sâu quan sát được (Chọn khoảng) <span class="text-danger">*</span>
                                            </label>
                                            <div class="d-flex flex-wrap gap-2" id="quantity-pill-group">
                                                <input type="hidden" name="quantity_range" id="quantity_range" value="6_20">
                                                <button type="button" class="btn-range-pill" data-val="unknown" onclick="selectQuantity('unknown', this)">Không xác định</button>
                                                <button type="button" class="btn-range-pill" data-val="1_5" onclick="selectQuantity('1_5', this)">1 – 5 con</button>
                                                <button type="button" class="btn-range-pill active" data-val="6_20" onclick="selectQuantity('6_20', this)">6 – 20 con</button>
                                                <button type="button" class="btn-range-pill" data-val="21_50" onclick="selectQuantity('21_50', this)">21 – 50 con</button>
                                                <button type="button" class="btn-range-pill" data-val="gt_50" onclick="selectQuantity('gt_50', this)">&gt; 50 con</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div id="fields-disease-group" class="p-3 bg-light rounded-4 border mb-3" style="display: none;">
                                    <div class="row g-3">
                                        <!-- Bộ phận bị bệnh -->
                                        <div class="col-md-6">
                                            <label for="affected_part" class="form-label fw-semibold text-dark small mb-1">
                                                Bộ phận cây trồng bị bệnh <span class="text-danger">*</span>
                                            </label>
                                            <select class="form-select form-select-sm rounded-3" id="affected_part" name="affected_part" disabled>
                                        ...
                                        <!-- Mức độ phát sinh -->
                                        <div class="col-md-6">
                                            <label for="severity_disease" class="form-label fw-semibold text-dark small mb-1">
                                                Mức độ gây hại <span class="text-danger">*</span>
                                            </label>
                                            <select class="form-select form-select-sm rounded-3" id="severity_disease" name="severity" disabled>
                                                <option value="none">Không có</option>
                                                <option value="low">Ít</option>
                                                <option value="medium" selected>Trung bình</option>
                                                <option value="high">Nhiều</option>
                                        ...
                                        <div class="col-12">
                                            ...
                                            <div class="d-flex flex-wrap gap-2" id="infection-pill-group">
                                                <input type="hidden" name="infection_rate_range" id="infection_rate_range" value="10_25" disabled>
                                                <button type="button" class="btn-range-pill" data-val="lt_5" onclick="selectInfection('lt_5', this)">&lt; 5%</button>
                                                <button type="button" class="btn-range-pill" data-val="5_10" onclick="selectInfection('5_10', this)">5 – 10%</button>
                                                <button type="button" class="btn-range-pill active" data-val="10_25" onclick="selectInfection('10_25', this)">10 – 25%</button>
                                                <button type="button" class="btn-range-pill" data-val="25_50" onclick="selectInfection('25_50', this)">25 – 50%</button>
                                                <button type="button" class="btn-range-pill" data-val="gt_50" onclick="selectInfection('gt_50', this)">&gt; 50%</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="mb-4">
                                <div class="d-flex align-items-center gap-2 mb-3">
                                    <span class="badge bg-primary text-white rounded-circle p-1" style="width: 24px; height: 24px; display: inline-flex; align-items: center; justify-content: center; font-size: 12px;">4</span>
                                ...
                                <div class="row g-3">
                                    <!-- Ảnh chụp thực địa -->
                                    <div class="col-md-5">
                                        <label class="form-label fw-semibold text-dark small mb-1">Ảnh chụp thực địa (Không bắt buộc)</label>
                                        <div class="d-flex flex-column gap-2">
                                            <div class="d-flex gap-2">
                                                <!-- Chụp từ Camera trên mobile -->
                                                <label class="btn btn-outline-secondary btn-sm rounded-3 d-flex align-items-center gap-1.5 cursor-pointer">
                                                    <i class="bi bi-camera-fill text-primary"></i> Chụp ảnh
                                                    <input type="file" name="image" id="survey-camera-input" accept="image/*" capture="environment" class="d-none" onchange="previewSurveyImage(this)">
                                                </label>
                                                <!-- Chọn từ file máy tính/thư viện -->
                                                <label class="btn btn-outline-secondary btn-sm rounded-3 d-flex align-items-center gap-1.5 cursor-pointer">
                                                    <i class="bi bi-image text-info"></i> Chọn từ máy
                                                    <input type="file" name="image" id="survey-file-input" accept="image/*" class="d-none" onchange="previewSurveyImage(this)">
                                                </label>
                                            </div>
                                            ...
                                            <div id="survey-image-preview" style="display: none;">
                                                <div class="image-preview-wrapper mt-2">
                                                    ...
                                                    <button type="button" class="image-preview-remove" onclick="removeSurveyImage()" title="Xóa ảnh">&times;</button>
                                                </div>
                                            </div>
                                            <small class="text-muted" style="font-size: 11px;">Hỗ trợ ảnh định dạng JPG, PNG, WEBP (Tối đa 10MB)</small>
                                        </div>
                                    </div>

                                    <!-- Ghi chú bổ sung -->
                                    <div class="col-md-7">
                                        <label for="notes" class="form-label fw-semibold text-dark small mb-1">Ghi chú quan sát / Ý kiến bổ sung</label>
                                        <textarea class="form-control form-control-sm rounded-3" id="notes" name="notes" rows="3" 
                                            placeholder="Nhập ghi chú quan sát cụ thể (ví dụ: phát hiện ngài bướm xuất hiện trong bẫy đèn, cây vải có hiện tượng rụng hoa...)" maxlength="1000"></textarea>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-end gap-2 pt-3 border-top">
                                <button type="reset" class="btn btn-light btn-sm px-3 rounded-3" onclick="resetSurveyForm()">Khôi phục</button>
                                <button type="submit" class="btn btn-primary btn-sm px-4 py-2 rounded-3 fw-bold shadow-sm" id="btn-submit-survey">
                                    <i class="bi bi-check2-circle me-1"></i> Lưu Khảo Sát & Gắn Snapshot IoT
                                </button>
                            </div>
                        </form>
                    @endif
                ...
                <div class="tab-pane fade" id="tab-survey-history" role="tabpanel">
                    <!-- BỘ LỌC LỊCH SỬ -->
                    <form method="GET" action="{{ route('degree-days.surveys.index') }}" class="row g-2 mb-3 align-items-end">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="object_type" class="form-select form-select-sm rounded-3">
                                ...
                                <option value="pest" {{ request('object_type') === 'pest' ? 'selected' : '' }}>Sâu đục cuống quả</option>
                                <option value="disease" {{ request('object_type') === 'disease' ? 'selected' : '' }}>Bệnh hại</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="severity" class="form-select form-select-sm rounded-3">
                                <option value="">-- Tất cả --</option>
                                <option value="none" {{ request('severity') === 'none' ? 'selected' : '' }}>Không có</option>
                                ...
                                <option value="high" {{ request('severity') === 'high' ? 'selected' : '' }}>Nhiều</option>
                                <option value="outbreak" {{ request('severity') === 'outbreak' ? 'selected' : '' }}>Bùng phát</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-2 d-flex gap-1.5">
                            <button type="submit" class="btn btn-primary btn-sm rounded-3 w-100"><i class="bi bi-funnel"></i> Lọc</button>
                            <a href="{{ route('degree-days.surveys.index') }}" class="btn btn-light btn-sm rounded-3" title="Xóa lọc"><i class="bi bi-arrow-counterclockwise"></i></a>
                        </div>
                    </form>
                            ...
                            <tbody>
                                @forelse ($surveys as $idx => $s)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="fw-bold text-dark">{{ $s->station->name ?? $s->station_name ?? '--' }}</div>
                                            <div class="text-muted small">[{{ $s->station->code ?? $s->station_code ?? '--' }}] {{ $s->station->garden->name ?? $s->garden_name ?? '' }}</div>
                                        </td>
                                        ...
                                        <td>
                                            @if ($s->object_type === 'pest')
                                                <span class="text-secondary font-monospace">{{ $s->quantity_range_label }}</span>
                                            @else
                                                <span class="text-secondary font-monospace">{{ $s->infection_rate_label }}</span>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @if ($s->iot_temperature !== null)
                                                <div class="small">
                                                    <span class="text-danger fw-bold"><i class="bi bi-thermometer-half"></i> {{ $s->iot_temperature }}°C</span> | 
                                                    <span class="text-info fw-medium"><i class="bi bi-droplet-half"></i> {{ $s->iot_humidity }}%</span>
                                                </div>
                                                <div class="text-muted" style="font-size: 11px;">
                                                    Gió: {{ $s->iot_wind_speed ?? 0 }} m/s | Mưa: {{ $s->iot_rainfall ?? 0 }} mm
                                                </div>
                                            @else
                                        ...
                                        <td style="text-align: center;">
                                            <button type="button" class="btn btn-outline-primary btn-sm px-2.5 py-1 rounded-3" 
    ...
    <div class="app-modal" id="modal-survey-detail">
        <div class="modal-dialog" style="max-width: 720px;">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-file-earmark-text-fill text-primary me-2"></i> Chi Tiết Bản Ghi Khảo Sát Thực Địa</h5>
            ...
            <div class="modal-body py-3">
                ...
                <div class="p-3 bg-light rounded-4 border mb-3 d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div>
                        ...
                        <div class="text-secondary small"><i class="bi bi-person me-1 text-primary"></i> Người khảo sát: <span id="m-surveyor" class="fw-semibold text-dark">--</span></div>
                    </div>
                    <div class="text-end">
                ...
                <div class="card border mb-3 rounded-3 p-3">
                    <h6 class="fw-bold text-dark mb-2.5"><i class="bi bi-binoculars-fill text-warning me-1.5"></i> Kết Quả Quan Sát Thực Địa:</h6>
                    <div class="row g-2 small">
                        <div class="col-sm-6">
                            <span class="text-muted">Đối tượng:</span> <strong id="m-object-type" class="text-dark">--</strong>
                        </div>
                        <div class="col-sm-6">
                            <span class="text-muted">Mức độ phát sinh:</span> <span id="m-severity" class="badge">--</span>
                        </div>
                        <div class="col-sm-6" id="m-pest-row">
                            <span class="text-muted">Giai đoạn & Số lượng:</span> <strong id="m-pest-info" class="text-dark">--</strong>
                        </div>
                        <div class="col-sm-6" id="m-disease-row">
                            <span class="text-muted">Bộ phận & Tỷ lệ:</span> <strong id="m-disease-info" class="text-dark">--</strong>
                        </div>
                    </div>
                    ...
                    <div class="mt-2 pt-2 border-top" id="m-image-box" style="display: none;">
                        ...
                        <a id="m-image-link" href="#" target="_blank">
                            <img id="m-image-img" src="" alt="Ảnh thực địa" class="rounded-3 border" style="max-height: 180px; max-width: 100%; object-fit: contain;">
                        </a>
                    </div>
                </div>
                ...
                <div class="card border rounded-3 p-3 bg-light">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <h6 class="fw-bold text-dark mb-0"><i class="bi bi-broadcast-pin text-primary me-1.5"></i> Snapshot Vi Khí Hậu IoT (Tại Thời Điểm Đo):</h6>
                        <span class="badge bg-white text-muted border font-monospace" id="m-iot-time" style="font-size: 11px;">--</span>
                    </div>
...
@push('scripts')
    <script>
        ...
        function fetchStationSnapshot() {
            ...
            fetch(`{{ route('degree-days.surveys.snapshot') }}?station_id=${stationId}&surveyed_at=${encodeURIComponent(surveyedAt)}`)
                .then(res => res.json())
                .then(res => {
                    if (res.success && res.data) {
                        ...
                        document.getElementById('iot-time-text').textContent = d.recorded_at_display || 'Chưa có bản ghi';
                        document.getElementById('iot-temp-val').textContent = vals.temp !== undefined ? (vals.temp + ' °C') : '--';
                        document.getElementById('iot-hum-val').textContent = vals.humidity !== undefined ? (vals.humidity + ' %') : '--';
                        document.getElementById('iot-rain-val').textContent = vals.rain !== undefined ? (vals.rain + ' mm') : '0.0 mm';
                        document.getElementById('iot-wind-val').textContent = vals.wind !== undefined ? (vals.wind + ' m/s') : '--';
                        document.getElementById('iot-soil-val').textContent = vals.soil_moist !== undefined ? (vals.soil_moist + ' %') : '--';
                        document.getElementById('iot-light-val').textContent = vals.light !== undefined ? Number(vals.light).toLocaleString() : '--';
                    }
                })
                .catch(err => {
            ...
            fetch(`{{ url('/degree-days/surveys') }}/${surveyId}`)
                ...
                .then(res => {
                    if (res.success && res.survey) {
                        const s = res.survey;
                        document.getElementById('m-station-name').textContent = `[${s.station_code}] ${s.station_name} (${s.garden_name})`;
                        ...
                        if (s.object_type === 'pest') {
                            ...
                            document.getElementById('m-pest-info').textContent = `${s.development_stage_label} (${s.quantity_range_label})`;
                        } else {
                            pestRow.style.display = 'none';
                            diseaseRow.style.display = 'block';
                            document.getElementById('m-disease-info').textContent = `${s.affected_part_label} (Tỷ lệ: ${s.infection_rate_label})`;
                        }
                        ...
                        // IoT Snapshot
                        const iot = s.iot || {};
                        document.getElementById('m-iot-time').textContent = iot.recorded_at ? ('Gói tin lúc ' + iot.recorded_at) : 'Chưa có dữ liệu trạm';
