...
@section('title', 'Thêm Trạm Quan Trắc IoT Mới')
...
@section('content')
    <x-page-header title="Thêm Trạm Quan Trắc IoT Mới" subtitle="Đăng ký mã trạm, vùng trồng và định vị tọa độ GIS cho trạm quan trắc hiện trường.">
        <x-slot:actions>
    ...
    <form action="{{ route('iot.stations.store') }}" method="POST">
        @csrf
        <div class="row g-4">
            <!-- CỘT BÊN TRÁI (6 COLS): THÔNG TIN CẤU HÌNH TRẠM -->
            <div class="col-lg-5">
                <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
                    <div class="form-section-title">
                        <i class="bi bi-cpu text-primary"></i> Thông Tin Cơ Bản Của Trạm
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold">Mã định danh trạm (Station Code) <span class="text-danger">*</span></label>
                        <input type="text" name="code" class="form-control font-monospace @error('code') is-invalid @enderror"
                            placeholder="Ví dụ: TPH-03, TLN-03..." value="{{ old('code') }}" required>
                        <div class="form-text small text-muted">Mã này phải khớp với cấu hình <code>STATION_CODE</code> trong code Python của trạm.</div>
                        @error('code')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                            placeholder="Ví dụ: Trạm Quan Trắc Vườn Vải Phúc Hòa Số 3..." value="{{ old('name') }}" required>
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Vùng trồng quản lý <span class="text-danger">*</span></label>
                        <select name="garden_id" class="form-select @error('garden_id') is-invalid @enderror" required>
                            <option value="">-- Chọn vùng trồng --</option>
                            @foreach ($gardens as $g)
                                <option value="{{ $g->id }}" {{ old('garden_id') == $g->id ? 'selected' : '' }}>
                                    {{ $g->name }} ({{ $g->code }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                            <option value="active" {{ old('status', 'active') == 'active' ? 'selected' : '' }}>Hoạt động (Hoạt động ổn định)</option>
                            <option value="danger" {{ old('status') == 'danger' ? 'selected' : '' }}>Cảnh báo dịch bệnh / Nguy hiểm</option>
                            <option value="maintenance" {{ old('status') == 'maintenance' ? 'selected' : '' }}>Đang bảo trì kỹ thuật</option>
                            <option value="inactive" {{ old('status') == 'inactive' ? 'selected' : '' }}>Tạm ngưng hoạt động</option>
                        </select>
                    </div>

                    <div class="mb-4">
                        <label class="form-label fw-semibold">Chu kỳ đọc & gửi dữ liệu (Giây)</label>
                        <div class="input-group">
                            <input type="number" name="data_interval" class="form-control font-monospace"
                                placeholder="900" value="{{ old('data_interval', 900) }}" min="5">
                            <span class="input-group-text bg-light text-muted">giây</span>
                        </div>
                        <div class="form-text small text-muted">
                            <i class="bi bi-info-circle me-1 text-info"></i> Khuyến nghị: <strong>900 giây (15 phút)</strong> cho vận hành thực tế, <strong>60 giây</strong> khi kiểm thử.
                        </div>
                    </div>

                    <div class="mt-auto pt-3 border-top d-flex gap-2">
                        <button type="submit" class="btn btn-primary px-4 py-2 fw-semibold">
                            <i class="bi bi-check-circle-fill me-1"></i> Lưu & Kích Hoạt Trạm
                        </button>
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="create-station-lat" name="latitude"
                                class="form-control font-monospace fw-bold text-primary"
                                value="{{ old('latitude', '21.054200') }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-semibold text-muted mb-1">Kinh độ (Longitude)</label>
                            <input type="text" id="create-station-lng" name="longitude"
                                class="form-control font-monospace fw-bold text-primary"
                                value="{{ old('longitude', '106.071200') }}">
                        </div>
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function initCreateMap() {
            const initialLat = parseFloat(document.getElementById('create-station-lat').value) || 21.0542;
            const initialLng = parseFloat(document.getElementById('create-station-lng').value) || 106.0712;

            mapPicker = L.map('map-create-picker').setView([initialLat, initialLng], 13);
            ...
            const updateCoordsFromCenter = () => {
                ...
                document.getElementById('create-station-lat').value = center.lat.toFixed(6);
                document.getElementById('create-station-lng').value = center.lng.toFixed(6);
            };
        ...
        function bindManualInputs() {
            ['create-station-lat', 'create-station-lng'].forEach(id => {
                const el = document.getElementById(id);
                if (el) {
                    el.addEventListener('input', function() {
                        isTypingCoords = true;
                        const lat = parseFloat(document.getElementById('create-station-lat').value);
                        const lng = parseFloat(document.getElementById('create-station-lng').value);
